...
import { Alert, Box, Heading, Text, FormControl, Modal, ModalBackdrop, 
  AlertText, Fab, FabLabel, FabIcon, ArrowLeftIcon, ScrollView } from "@gluestack-ui/themed";
  ...
  return (
    <Box flex={1} backgroundColor="$white" justifyContent="center">
      ...
      mt={"$8"} mx={"$3"} px={"$3"} pt={"$2"}>
        ...
        <FormControl>
          <Input label={"Title"} width={"$full"} height={"$10"} value={title} 
          onChangeText={(title) => setTitle(title)} />
          <Input textarea={true} label="Content" width={"$full"} height={"$32"} 
          value={content} onChangeText={(content) => setContent(content)} />
